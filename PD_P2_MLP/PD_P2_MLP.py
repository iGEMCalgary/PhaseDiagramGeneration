...
def formatData(phaseDf):
	...
	uniquePhases.sort()
	phaseCol = [phasePoint/1.-1 for phasePoint in phaseCol]
...
def getModel(trainX, trainY, uniquePhases, customSettings={}):
	...
	#Else build the model from scratch
		...
		phaseModel, trainingHistoryDf = buildAndTrainModel(phaseModel, trainX, trainY, modelSavePath, trainingHistoryPath, customSettings)
...
#Train the model from scratch
def buildAndTrainModel(phaseModel, trainX, trainY, modelSavePath, trainingHistoryPath, customSettings):
	
	#Specify settings of the model training
	defaultSettings = {'trainingMaxEpochs': 20000,
	...
	while round(i,5) <= 1.0:		
		while round(j,5) <= round(1.0 - i, 5):
			...
			generatedTable[4].append(round(getPrediction(phaseModel.predict(
									 pd.DataFrame([[i,j,1.0-i-j,tempConst]]))[0])[0],5)+1)
			j = j + denseInterval
		j = 0
		i = i + denseInterval
...
phaseModel = getModel(trainDataX, trainDataY, uniquePhases, custSettings)
